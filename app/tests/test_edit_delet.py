...
from app.models import ReservaEquipamento, ReservaSala
...
# edição de sala
@pytest.mark.django_db
def test_editar_reserva_sala(client, sala):
    ...
    url = reverse('editar_reserva_sala', args=[reserva.pk])
    data = {'data': '2025-10-16', 'hora_inicio': '10:00', 'hora_fim': '11:00'}
    response = client.post(url, data, follow=True)
    reserva.refresh_from_db()
    assert reserva.data.strftime('%Y-%m-%d') == '2025-10-16'
...
@pytest.mark.django_db
def test_deletar_reserva_sala(client, sala):
    reserva = ReservaSala.objects.create(
        sala= sala,
        data='2025-10-15',
        hora_inicio='08:00',
    ...
    url = reverse('deletar_reserva_sala', args=[reserva.pk])
    response = client.post(url, follow=True)
    assert ReservaSala.objects.count() == 0
    assert response.status_code == 200